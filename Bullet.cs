...
public class Bullet1:MonoBehaviour
{
	...
	protected virtual void complete()
	{
        ...
        if(c != null && c.BloodUIControllers != null)
        {
			if (player != null)
			{
				HeroController play = player.GetComponent<HeroController>();
				if (play != null)
				{
					c.attackedWithDamage(damage, play);
				}
				else
				{
					c.attackedWithDamage(damage, null);
				}
			}
			else
			{
		...
		else if (target.CompareTag("null"))
		{
